...
def train_mini_batch(X_train, Y_train, X_valid, Y_valid,
    ...
    with tf.Session() as sess:
        ...
        last_batch_size = len(X_train) % batch_size
        ...
        for epoch in range(epochs):
            ...
            for step in range(num_batches):
                ...
                if last_batch_size != 0 and step == num_batches - 1:
                    X_batch = X_shuffled[step * last_batch_size:
                                         (step + 1) * last_batch_size]
                    Y_batch = Y_shuffled[step * last_batch_size:
                                         (step + 1) * last_batch_size]
                    sess.run(
                        train_op,
                        feed_dict={x: X_batch, y: Y_batch}
