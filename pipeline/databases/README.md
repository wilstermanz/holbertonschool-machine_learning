...
In this project, we touch the first 2: relation and not relation database.

Relation databases are mainly used for application, not for source of data for training your ML models, but it can be really useful for the data processing, labeling and injection in another data storage. In this project, we will play with basic SQL commands but also create automation and computing on your data directly in SQL - less load at your application level since the computing power is dispatched to the database.

Not relation databases, known as NoSQL, will give us flexibility on our data: document, versioning, not a fix schema, no validation to improve performance, complex lookup, etc.
...
### General
-   All SQL files were executed on Ubuntu 16.04 LTS (or 18.04) using `MySQL 5.7` (version 5.7.30)
-   All Mongo files were be interpreted/compiled on Ubuntu 16.04 LTS (or 18.04) using `MongoDB` (version 4.2)
-   All Python files were interpreted/compiled on Ubuntu 16.04 LTS (or 18.04) using `python3` (version 3.5 or 3.7) and `PyMongo` (version 3.10)
...
[Official installation guide](https://www.mongodb.com/docs/manual/installation/ "Official installation guide")
...
Import the database `hbtn_0d_tvshows_rate.sql` dump to your MySQL server.
...
Import the database dump from `hbtn_0d_tvshows_rate.sql` to your MySQL server.
